...
— everything that CAN live below the paint path SHOULD). Toolkit-neutral by
construction — no Qt imports, core-shaped seams — so a future non-Qt front
end can lift it into a real hub core lib (spine absorption 12f342f1, rider
2). Absorbed from the Textual hub as a frozen copy-split; the stage-TUI
resolver stayed behind (the Qt hub spawns via launch.resolve_stage_app
instead). Composes the graph
...
cjm_transcript_correction_core.spine's rungs (N=2, decomp-TUI discovery.py
...
async def open_stack(
    ...
    *, manifests_dir: str = ".cjm/manifests",  # Capability manifests directory
    ...
    CARRIED COPY of cjm_transcript_correction_core.spine.open_stack (2ce81638
